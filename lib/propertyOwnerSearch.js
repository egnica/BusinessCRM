...
export function getMailingContactName(record = {}) {
  const candidates = [
    record.mailingContactName,
    record.taxNameRaw,
    record.TAX_NAME,
    record.mailingAddress?.recipientName,
  ]
    .map(clean)
    .filter(Boolean);
  ...
  for (const candidate of candidates) {
    ...
    const looksLikeOrganization =
      /^\d/.test(candidate) ||
      /\b(?:LLC|L\.?L\.?C\.?|INC|INCORPORATED|CORP|CORPORATION|LTD|LIMITED|LLP|LP|PLC|PLLC|COMPANY|PROPERTIES|PROPERTY|REALTY|INVESTMENTS?|HOLDINGS?|GROUP|PARTNERSHIP|ASSOCIATION|FOUNDATION|BANK|CHURCH|UNIVERSITY|AUTHORITY|TRUST|TRUSTEES?)\b/i.test(
        candidate,
      ) ||
      /^(?:CITY|COUNTY|STATE|TOWNSHIP|VILLAGE) OF\b/i.test(candidate);

    if (!looksLikeOrganization && /[A-Z]/i.test(upper)) {
      return candidate;
    }
  }
...
function normalizeFilters(filters = {}) {
  ...
  const onlyWithMailingName =
    filters.onlyWithMailingName === true ||
    filters.onlyWithMailingName === "true";
...
async function fetchCountyCitySummaries(county, filters) {
  ...
  while (summaries.length < filters.maxResults) {
    const remaining = filters.maxResults - summaries.length;
    const requestCount = Math.min(PAGE_SIZE, remaining);
    ...
    const data = await arcgisQuery(layerId, {
      ...
      resultRecordCount: String(requestCount),
    });
    ...
    for (const feature of features) {
      ...
      if (
        ...
        (!filters.onlyWithMailingName || summary.mailingContactName)
      ) {
    ...
    if (features.length < requestCount) break;
  }

  return summaries.slice(0, filters.maxResults);
}
...
async function enrichSearchResultsWithMetro(prospects) {
  if (!prospects.length) return [];

  const ownerNames = [
    ...new Set(
      prospects.map((prospect) => prospect.ownerNameRaw).filter(Boolean),
    ),
  ];
  ...
  const rows = (
    await Promise.all(
      COUNTY_OPTIONS.map((county) =>
        fetchRowsByOwnerNames(county, ownerNames),
      ),
    )
  ).flat();

  const prospectsByOwnerName = new Map();

  for (const prospect of prospects) {
    const ownerNameKey = normalizedKey(prospect.ownerNameRaw);
    if (!ownerNameKey) continue;

    const existing = prospectsByOwnerName.get(ownerNameKey) || [];
    existing.push(prospect);
    prospectsByOwnerName.set(ownerNameKey, existing);
  }

  const confirmedByProspect = new Map();
  ...
  for (const prospect of prospects) {
    ...
    for (const property of prospect.properties || []) {
      if (!property?.parcelKey) continue;
      byParcel.set(property.parcelKey, property);
    }

    confirmedByProspect.set(prospect.propertyOutreachKey, byParcel);
  }
  ...
  for (const row of rows) {
    ...
    const ownerNameKey = normalizedKey(attributes.OWNER_NAME);
    const candidates = prospectsByOwnerName.get(ownerNameKey) || [];
    if (!candidates.length) continue;

    const rowMailingKey = mailingKey(buildMailingAddress(attributes));
    if (!rowMailingKey) continue;

    const property = toProperty(attributes, row.county);
    if (!property.parcelKey) continue;

    for (const prospect of candidates) {
      const prospectMailingKey = mailingKey(prospect.mailingAddress);
      if (!prospectMailingKey || prospectMailingKey !== rowMailingKey) {
        continue;
      }

      confirmedByProspect
        .get(prospect.propertyOutreachKey)
        ?.set(property.parcelKey, property);
    }
  }

  return prospects.map((prospect) => {
    const confirmedProperties = [
      ...(confirmedByProspect.get(prospect.propertyOutreachKey)?.values() || []),
    ];
    const metroAssessedValue = confirmedProperties.reduce(
    ...
    return {
      ...prospect,
      metroPropertyCount: confirmedProperties.length,
      metroAssessedValue,
      metroCityBreakdown: cityBreakdown(confirmedProperties),
      metroIdentityConfirmed: Boolean(mailingKey(prospect.mailingAddress)),
    };
  });
}
...
export async function searchPropertyOwners(
  ...
  const summaries = mergeCitySummaries(summaryGroups, filters).slice(
    0,
    filters.maxResults,
  );
  ...
  let prospects = selectedSummaries;

  if (options.includeDetails !== false) {
    const cityProspects = await enrichCitySummaries(
      selectedSummaries,
      filters.city,
    );
    prospects = await enrichSearchResultsWithMetro(cityProspects);
  }
