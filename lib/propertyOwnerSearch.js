...
function normalizeFilters(filters = {}) {
  ...
  const minPortfolioSize =
    filters.minPortfolioSize === "" ||
    filters.minPortfolioSize == null
      ? null
      : Math.round(clampNumber(filters.minPortfolioSize, 1, 1, 1000));

  const maxPortfolioSize =
    filters.maxPortfolioSize === "" ||
    filters.maxPortfolioSize == null
      ? null
      : Math.round(clampNumber(filters.maxPortfolioSize, 1, 1, 1000));

  if (
    minPortfolioSize != null &&
    maxPortfolioSize != null &&
    minPortfolioSize > maxPortfolioSize
  ) {
    throw new Error(
      "Portfolio minimum cannot be greater than portfolio maximum.",
    );
  }
...
function groupRows(rows, filters) {
  ...
  return [...groups.values()]
    ...
    .filter((prospect) => {
      if (
        filters.minPortfolioSize != null &&
        prospect.propertyCount < filters.minPortfolioSize
      ) {
        return false;
      }
      ...
      if (
        filters.maxPortfolioSize != null &&
        prospect.propertyCount > filters.maxPortfolioSize
      ) {
        return false;
      }
