...
function normalizeFilters(filters = {}) {
  const county = COUNTY_LAYERS[filters.county] ? filters.county : "Hennepin";
...
async function arcgisQuery(layerId, params) {
  ...
  if (!response.ok) {
    throw new Error(`Parcel source request failed: ${response.status}`);
  }

  const data = await response.json();

  if (data.error) {
    throw new Error(data.error.message || "MetroGIS parcel query failed");
  }
...
export async function searchPropertyOwners(
  ...
  const rows = await fetchSourceRows(filters);
  const prospects = groupRows(rows, filters);
  ...
  if (!paginate) {
    const maxOwners = Math.round(
      clampNumber(options.maxOwners, total || 1, 1, 5000),
    );

    return {
      filters,
      sourcePropertyCount: rows.length,
      total,
